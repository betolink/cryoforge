    ...
    while True:
        ...
        if "Contents" in response:
            ...
            filtered_page = [
                ...
                if filename_pattern.match(obj["Key"].split("/")[-1])
                and obj["Key"].endswith(".nc")
            ]
...
def s3_path_to_local_path(s3_path, cache_root="/tmp/duck_cache"):
    ...
    local_path = os.path.join(
        cache_root, bucket, prefix_and_file.replace("**/*.parquet", "")
    )
    return local_path
...
def cache_parquet_file(
    ...
    for s3_path in s3_paths:
        ...
        if not os.path.exists(local_path):
            ...
            logger.debug(
                f"Downloading {len(matching_files)} parquet files to: {local_path}"
            )
...
def get_overlapping_grid_names(
    ...
    if partition_type == "latlon":
        ...
        logger.debug(
            f"Found {len(grids_hex)} H3 grids for geometry: {geojson_geometry}"
        )
