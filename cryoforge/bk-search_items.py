...
def search_duckstac(
    catalog: str = "s3://its-live-data/test-space/stac/geoparquet/h3r2", args: dict = {}
):
    filters = [
        {
            "op": ">=",
            "args": [{"property": "percent_valid_pixels"}, args.percent_valid_pixels],
        },
        {"op": "=", "args": [{"property": "proj:code"}, args.epsg]}
        if args.epsg
        else {},
    ]
    if args.geojson:
    ...
    print(f"Searching DuckSTAC with args: {search_args}")
...
def search_rustac(
    catalog: str = "s3://its-live-data/test-space/stac/geoparquet/latlon",
    args: dict = {},
):
...
def search_items():
    ...
    parser.add_argument(
        "--query-engine",
        help="Query engine to use (duckstac, rustac, or pystac_client)",
        default="pystac_client",
    )
    parser.add_argument(
        "--granule", help="URL of an overlapping ITS_LIVE .nc granule file"
    )
    parser.add_argument(
        "--bbox", help="Bounding box in the format 'lon_min,lat_min,lon_max,lat_max'"
    )
    parser.add_argument(
        "--geojson", help="Geojson file with a geometry type to filter items"
    )
    parser.add_argument(
        "--datetime",
        help="Datetime range in STAC format: 'YYYY-MM-DDTHH:MM:SSZ/YYYY-MM-DDTHH:MM:SSZ'",
    )
    parser.add_argument(
        "--cache", help="Cache geoparquet files in disk if present", action="store_true"
    )
    parser.add_argument(
        "--max-items",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Filter items by minimum percent valid pixels (e.g., 90)",
    )
    ...
    try:
        if args.granule:
            ...
            try:
                with open(args.geojson, "r") as f:
                    geom = json.loads(f.read())["geometry"]
                bbox = geom["coordinates"][
                    0
                ]  # Assuming the first polygon's coordinates
                bbox = [
